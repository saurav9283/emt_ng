...
module.exports = {
  ...
  getCurrentMatches: async (league, season, date) => {
    ...
    try {
      ...
      const modifiedResponse = await response.data.response.map(transformFixture);
      ...
      return { error: error.response?.data, matches: "API ERROR GETTING CURRENT DAY MATCHES" };
    }
  },
};
